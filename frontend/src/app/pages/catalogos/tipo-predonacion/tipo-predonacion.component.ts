...
import { TipoPredonacionModalComponent } from './tipo-predonacion-modal.component';
import { TipoPredonacionDialogComponent} from './tipo-predonacion-dialog.component';
import { ToasterService, ToasterConfig, Toast, BodyOutputType } from 'angular2-toaster';
import 'style-loader!angular2-toaster/toaster.css';

@Component({
  selector: 'ngx-tipo-predonacion',
  styleUrls: ['./tipo-predonacion.component.scss'],
  templateUrl: './tipo-predonacion.component.html',
})
export class TipoPredonacionComponent {
  settings = {
    mode : 'external',
    actions : {
        position : 'right',
  ...
  constructor(private service: TipoPredonacionService, private modalService: NgbModal, private toasterService: ToasterService) {  }
  ...
  nuevoRegistro() {
    this.modalRef = this.modalService.open(TipoPredonacionModalComponent, { size: 'lg', container: 'nb-layout' });

    this.modalRef.componentInstance.modalHeader = 'Nuevo Tipo de Donación';
    this.modalRef.componentInstance.item = new TipoPredonacion();
  ...
  editarRegistro(value) {
    ...
    this.service.findById(value.data.cod_predonacion)
        .subscribe(
        data => {
            this.modalRef.componentInstance.modalHeader = 'Editar Tipo de Predonación';
  ...
  eliminarRegistro(value) {
    this.modalRef = this.modalService.open(TipoPredonacionDialogComponent, { size: 'lg', container: 'nb-layout' });
    ...
    this.modalRef.result.then((data) => {
      this.service.eliminarPorId(value.data.cod_predonacion)
          .subscribe(
          data => {
            if (data){
              this.showToast("info", "Tipo de predonación", "Registro eliminado exitosamente.");
              this.getAll();        
            }else{
