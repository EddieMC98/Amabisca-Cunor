...
import { GrupoEtnicoModalComponent } from './grupo-etnico-modal.component';
import { GrupoEtnicoDialogComponent} from './grupo-etnico-dialog.component';
import { ToasterService, ToasterConfig, Toast, BodyOutputType } from 'angular2-toaster';
import 'style-loader!angular2-toaster/toaster.css';

@Component({
  selector: 'ngx-grupo-etnico',
  styleUrls: ['./grupo-etnico.component.scss'],
...
export class GrupoEtnicoComponent {
  ...
  constructor(private service: GrupoEtnicoService, private modalService: NgbModal, private toasterService: ToasterService) {  }
  ...
  nuevoRegistro() {
    this.modalRef = this.modalService.open(GrupoEtnicoModalComponent, { size: 'lg', container: 'nb-layout' });

    this.modalRef.componentInstance.modalHeader = 'Nuevo Grupo Étnico';
    this.modalRef.componentInstance.item = new GrupoEtnico();
  ...
  editarRegistro(value) {
    ...
    this.service.findById(value.data.cod_grupo_etnico)
        .subscribe(
        data => {
            this.modalRef.componentInstance.modalHeader = 'Editar Grupo Étnico';
  ...
  eliminarRegistro(value) {
    this.modalRef = this.modalService.open(GrupoEtnicoDialogComponent, { size: 'lg', container: 'nb-layout' });
    ...
    this.modalRef.result.then((data) => {
      this.service.eliminarPorId(value.data.cod_grupo_etnico)
          .subscribe(
          data => {
            if (data){
              this.showToast("info", "Grupo Étnico", "Registro eliminado exitosamente.");
              this.getAll();        
            }else{
