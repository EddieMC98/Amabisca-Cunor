...
import { TipoTransfusionModalComponent } from './tipo-transfusion-modal.component';
import { TipoTransfusionDialogComponent} from './tipo-transfusion-dialog.component';
import { ToasterService, ToasterConfig, Toast, BodyOutputType } from 'angular2-toaster';
import 'style-loader!angular2-toaster/toaster.css';

@Component({
  selector: 'ngx-tipo-transfusion',
  styleUrls: ['./tipo-transfusion.component.scss'],
  templateUrl: './tipo-transfusion.component.html',
})
export class TipoTransfusionComponent {
  settings = {
    mode : 'external',
    actions : {
        position : 'right',
  ...
  constructor(private service: TipoTransfusionService, private modalService: NgbModal, private toasterService: ToasterService) {  }
  ...
  nuevoRegistro() {
    this.modalRef = this.modalService.open(TipoTransfusionModalComponent, { size: 'lg', container: 'nb-layout' });

    this.modalRef.componentInstance.modalHeader = 'Nuevo Tipo de Transfusión';
    this.modalRef.componentInstance.item = new TipoTransfusion();
  ...
  editarRegistro(value) {
    ...
    this.service.findById(value.data.cod_transfusion)
        .subscribe(
        data => {
            this.modalRef.componentInstance.modalHeader = 'Editar Tipo de Transfusión';
  ...
  eliminarRegistro(value) {
    this.modalRef = this.modalService.open(TipoTransfusionDialogComponent, { size: 'lg', container: 'nb-layout' });
    ...
    this.modalRef.result.then((data) => {
      this.service.eliminarPorId(value.data.cod_transfusion)
          ...
          data => {
            if (data){
              this.showToast("info", "Tipo de Transfusión", "Registro eliminado exitosamente.");
              this.getAll();        
            }else{
