...
import { IntercambioModalComponent } from './intercambio-modal.component';
import { IntercambioDialogComponent} from './intercambio-dialog.component';
import { ToasterService, ToasterConfig, Toast, BodyOutputType } from 'angular2-toaster';
import 'style-loader!angular2-toaster/toaster.css';
import { DatePipe } from '@angular/common';

@Component({
  selector: 'ngx-intercambio',
...
export class IntercambioComponent {
  ...
  private items: UnidadIntercambio[];
  ...
  constructor(private service: UnidadIntercambioService, private modalService: NgbModal, private toasterService: ToasterService) {  }
  ...
  nuevoRegistro() {
    this.modalRef = this.modalService.open(IntercambioModalComponent, { size: 'lg', container: 'nb-layout' });

    this.modalRef.componentInstance.modalHeader = 'Nuevo Intercambio';
    this.modalRef.componentInstance.item = new UnidadIntercambio();
  ...
  editarRegistro(value) {
    ...
    this.service.findById(value.data.cod_unidad_intercambio)
        .subscribe(
        data => {
  ...
  eliminarRegistro(value) {
    this.modalRef = this.modalService.open(IntercambioDialogComponent, { size: 'lg', container: 'nb-layout' });
    ...
    this.modalRef.result.then((data) => {
      this.service.eliminarPorId(value.data.cod_unidad_intercambio)
          .subscribe(
          data => {
            if (data){
              this.showToast("info", "Intercambio", "Registro eliminado exitosamente.");
              this.getAll();        
            }else{
