...
import { DonacionExternaModalComponent } from './donacion-externa-modal.component';
import { DonacionExternaDialogComponent} from './donacion-externa-dialog.component';
import { ToasterService, ToasterConfig, Toast, BodyOutputType } from 'angular2-toaster';
import 'style-loader!angular2-toaster/toaster.css';
import { DatePipe } from '@angular/common';

@Component({
  selector: 'ngx-donacion-externa',
  styleUrls: ['./donacion-externa.component.scss'],
  templateUrl: './donacion-externa.component.html',
})
export class DonacionExternaComponent {
  source: LocalDataSource = new LocalDataSource();
  private items: DonacionExterna[];
  ...
  constructor(private service: DonacionExternaService, private modalService: NgbModal, private toasterService: ToasterService) {  }
  ...
  nuevoRegistro() {
    this.modalRef = this.modalService.open(DonacionExternaModalComponent, { size: 'lg', container: 'nb-layout' });

    this.modalRef.componentInstance.modalHeader = 'Nuevo Registro';
    this.modalRef.componentInstance.item = new DonacionExterna();
  ...
  editarRegistro(value) {
    ...
    this.service.findById(value.data.cod_donacion_ext)
        .subscribe(
        data => {
  ...
  eliminarRegistro(value) {
    this.modalRef = this.modalService.open(DonacionExternaDialogComponent, { size: 'lg', container: 'nb-layout' });
    ...
    this.modalRef.result.then((data) => {
      this.service.eliminarPorId(value.data.cod_donacion_ext)
          .subscribe(
          data => {
            if (data){
              this.showToast("info", "Donación Externa", "Registro eliminado exitosamente.");
              this.getAll();        
            }else{
