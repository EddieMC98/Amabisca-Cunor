...
import { TransNoUtilizadaModalComponent } from './trans-no-utilizada-modal.component';
import { TransNoUtilizadaDialogComponent} from './trans-no-utilizada-dialog.component';
...
@Component({
  selector: 'ngx-trans-no-utilizada',
  styleUrls: ['./trans-no-utilizada.component.scss'],
  templateUrl: './trans-no-utilizada.component.html',
})
export class TransNoUtilizadaComponent {
  source: LocalDataSource = new LocalDataSource();
  private items: TransNoUtilizada[];
  ...
  constructor(private service: TransNoUtilizadaService, private modalService: NgbModal, private toasterService: ToasterService) {  }
  ...
  nuevoRegistro() {
    this.modalRef = this.modalService.open(TransNoUtilizadaModalComponent, { size: 'lg', container: 'nb-layout' });

    this.modalRef.componentInstance.modalHeader = 'Nuevo Registro';
    this.modalRef.componentInstance.item = new TransNoUtilizada();
  ...
  editarRegistro(value) {
    ...
    this.service.findById(value.data.cod_trans_no_util)
        .subscribe(
        data => {
  ...
  eliminarRegistro(value) {
    this.modalRef = this.modalService.open(TransNoUtilizadaDialogComponent, { size: 'lg', container: 'nb-layout' });
    ...
    this.modalRef.result.then((data) => {
      this.service.eliminarPorId(value.data.cod_trans_no_util)
          .subscribe(
          data => {
            if (data){
              this.showToast("info", "Transfusiones no utilizadas", "Registro eliminado exitosamente.");
              this.getAll();        
            }else{
